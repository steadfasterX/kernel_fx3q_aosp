...
EXPORT_SYMBOL(boost_enable_flag);
#endif

#ifdef CONFIG_MACH_LGE_FX3Q_TMUS
extern int get_usbin_with_ta(void);
int check_vbatt;
EXPORT_SYMBOL(check_vbatt);
bool check_vbatt_flag = false;
EXPORT_SYMBOL(check_vbatt_flag);
extern bool insert_ta_flag;
#endif
...
static void bms_soc_monitor_work(struct work_struct *work)
{
	...
	int i;
	int vbat_avg_mv = 0;
	int vbat_avg_num_samples = CHECK_COUNT;
	static int check_is_not_finished;
	static int before_check_state;
	static int check_boost_ic_threshold;
#endif

#ifdef CONFIG_MACH_LGE_FX3Q_TMUS
	static int before_vbatt;
	static int check_vbatt_counter;
#endif
...
#ifdef CONFIG_MACH_LGE_FX3Q_TMUS
	if (insert_ta_flag) {
		before_vbatt = check_vbatt;
 		check_vbatt = vbatt;

		if (get_usbin_with_ta() < 4450) {
			check_vbatt_counter++;
			pr_info("[CHECK][TA] Counter : %d\n", check_vbatt_counter);
			if (check_vbatt_counter > 3) {
				if (check_vbatt < before_vbatt)
					check_vbatt_flag = true;
			}
		}
	} else {
		check_vbatt_flag = false;
		check_vbatt_counter = 0;
	}
#endif

#ifdef CONFIG_LGE_PM_BOOST_IC
	if ((vbatt/1000) <= BOOST_ENABLE_THRESHOLD)
		check_is_not_finished = 1;

	else
		check_is_not_finished = 0;

	if (check_is_not_finished != before_check_state) {

		for (i = 0; i < CHECK_COUNT; i++) {
			check_boost_ic_threshold = vbatt / 1000;
			vbat_avg_mv += check_boost_ic_threshold;
			msleep(200);
		}
	...
	before_check_state = check_is_not_finished;
#endif
...
int pm8921_bms_get_percent_charge(void)
{
   ...
   /* block this code becuase of Batt Log issue
    * donghyuk79.park@lge.com 2013.04.12
    *
	if (calculated_soc == 0 )
	{
		pr_info("[BMS-POWER] calculated_soc ==0, count = %d \n",soc_zero_count);
		soc_zero_count++;
		if (soc_zero_count > 5)
		{
			return 0;
...
//[LGE_CHANGE_S] 2013.03.01 daewon1004.kim@lge.com l9ii change the BL53QH battery(G2 modle), so i add BL53QH profile (LGC).
	...
	*/
	...
	pr_info("[BMS] Battery profile data is set to LGE_FX3_BL-59JH-LGE_2460mAh, id:%d\n", lge_battery_info);
	chip->fcc = LGE_BL_59JH_LGC_2460mAh_data.fcc;
	chip->fcc_temp_lut = LGE_BL_59JH_LGC_2460mAh_data.fcc_temp_lut;
	chip->fcc_sf_lut = LGE_BL_59JH_LGC_2460mAh_data.fcc_sf_lut;
	chip->pc_temp_ocv_lut = LGE_BL_59JH_LGC_2460mAh_data.pc_temp_ocv_lut;
	chip->pc_sf_lut = LGE_BL_59JH_LGC_2460mAh_data.pc_sf_lut;
	chip->rbatt_sf_lut = LGE_BL_59JH_LGC_2460mAh_data.rbatt_sf_lut;
	chip->default_rbatt_mohm = LGE_BL_59JH_LGC_2460mAh_data.default_rbatt_mohm;
	chip->delta_rbatt_mohm = LGE_BL_59JH_LGC_2460mAh_data.delta_rbatt_mohm;
#else
...
no_data:
	...
	pr_info("[BMS] Battery profile data is set to LGC_BL53QH_2000_2150mAh, id:%d (default) \n", lge_battery_info);
