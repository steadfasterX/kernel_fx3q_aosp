...
#endif
#if defined(CONFIG_MACH_LGE_FX3_VZW) || defined(CONFIG_MACH_LGE_F6_TMUS)|| defined(CONFIG_MACH_LGE_F6_VDF)
...
//AICL init variable
static bool is_finished_check = false;
static int step_down_before_finished = 0;

extern int check_vbatt;
extern bool check_vbatt_flag;
bool insert_ta_flag = false;
EXPORT_SYMBOL(insert_ta_flag);
#endif
...
int get_usbin_with_ta(void) {
	...
	vin_now = result.physical;
	pr_info("[CHECK] USBIN = %lld, meas = 0x%llx, vin_now = %d\n", result.physical,
						result.measurement, vin_now);
	return vin_now/1000;
}
EXPORT_SYMBOL(get_usbin_with_ta);
...
void pm8921_charger_vbus_draw(unsigned int mA)
{
	...
	if (fsm_state == FSM_STATE_TRKL_CHG_8) {
		trklchg_current_enable_flag = true;
		usb_dc_mA_in_sdp = 500;
		pr_info(" usb : %d, TA : %d \n",usb_online, ac_online);

		if (usb_online) {
			for(counter = 0; counter < 10; counter++) {
				++counter;
				the_chip->trkl_current = counter * 10;	// increase +10mA in USB
			}
			pr_info("[TRKL_CHG] Cable type : Insert USB, fsm_state = %d, trkl_current = %d \n", fsm_state, the_chip->trkl_current);
		} else {
			for (counter = 0; counter < 3; counter++) {
				++counter;
				the_chip->trkl_current = counter * 50;	// increase +50mA in TA
			}
			pr_info("[TRKL_CHG]Cable type : Insert TA, fsm_state = %d, trkl_current = %d \n", fsm_state, the_chip->trkl_current);
		}
...
/* 2012-04-20, hiro.kwon@lge.com */
	...
	if (usb_present) {
		...
		is_finished_check = false;
		step_down_before_finished = 0;
		insert_ta_flag = false;
		check_vbatt_flag = false;
#endif
#if defined(CONFIG_MACH_LGE_L9II_COMMON)
/* BEGIN : dukwung.kim@lge.com 2013-07-01 power off charging mode flag */
...
#ifdef CONFIG_MACH_LGE_FX3Q_TMUS
			pm8xxx_adc_read(CHANNEL_USBIN, &Vin_now);
			pr_info("[AICL][Step Down] Vin_min = %d, Vin = %lld, before = %d\n", chip->vin_min*1000,Vin_now.physical, usb_ma);

			if(!is_finished_check) {
				step_down_before_finished++;

				if(step_down_before_finished == 5) {
					if(Vin_now.physical < chip->vin_min * 1000) {
						for(i = 0; i < step_down_before_finished; i++) {
							Vin_now_check += Vin_now.physical;
							msleep(200);
						}

						Vin_now_check = Vin_now_check / step_down_before_finished;
				...
				pr_info("[AICL][Step Down] is_finished_check = %d, step_down_before_finished = %d\n", is_finished_check, step_down_before_finished);
...
#ifdef CONFIG_MACH_LGE_FX3Q_TMUS
			pr_info("[AICL] usb_now=%d, usb_target = %d\n",
				usb_ma, usb_target_ma);
#endif
		}
	}
...
#ifdef CONFIG_MACH_LGE_FX3Q_TMUS
			pm8xxx_adc_read(CHANNEL_USBIN, &Vin_now);
			pr_info("[AICL][Step Up] Vin_min = %d Vin = %lld, Before_usb_ma = %d \n", chip->vin_min*1000, Vin_now.physical, usb_ma);
#endif
			increase_usb_ma_value(&usb_ma);
			__pm8921_charger_vbus_draw(usb_ma);
#ifdef CONFIG_MACH_LGE_FX3Q_TMUS
//			is_finished_check = true;
			pr_info("[AICL][Step Up] After_usb_ma = %d, Final MAX Target = %d \n", usb_ma, usb_target_ma);
#endif
#ifdef CONFIG_LGE_PM_VZW_FAST_CHG
...
#ifdef CONFIG_MACH_LGE_FX3Q_TMUS			
			is_finished_check = true;
			pr_info("[AICL][Step Up] Final usb_ma = %d\n", usb_ma);
#endif
		}
	}
...
#ifdef CONFIG_MACH_LGE_FX3Q_TMUS
#define XO_TEMP_CHECK_COUNTER 3
#endif
...
#ifdef CONFIG_LGE_PM 
	pr_info("[Thermal] thermal_mitigation=%d, ibatmax=%d\n"
		, thermal_mitigation
		, chip->thermal_mitigation[thermal_mitigation]);
#endif

#ifdef CONFIG_MACH_LGE_FX3Q_TMUS
	for(i = 0; i < XO_TEMP_CHECK_COUNTER; i++) {
		xo_temp_check = (int)xo_therm.physical / 1000;
		xo_temp_avg += xo_temp_check;
		msleep(200);
	}
	xo_temp_avg = xo_temp_avg / XO_TEMP_CHECK_COUNTER;
...
#ifdef CONFIG_MACH_LGE_FX3Q_TMUS
#define TRKL_BATTERY_THRESHOLD 3000
#endif
...
#ifdef CONFIG_MACH_LGE_FX3Q_TMUS
	static int batt_trkl_threshold;

	batt_trkl_threshold = get_prop_battery_uvolts(chip) / 1000;
#endif
#endif
...
#ifdef CONFIG_LGE_PM_TRKLCHG_IN_KERNEL
{
		...
		if ((is_usb_chg_plugged_in(chip) && boot_reason == PWR_ON_EVENT_USB_CHG) || (boot_reason == PWR_ON_EVENT_WALL_CHG && is_dc_chg_plugged_in(chip))) {
			...
			trklchg_current_enable_flag = true;
			usb_dc_mA_in_sdp = 500;
			pr_info("[TRKL_CHG] chg_plugged_in = %d, dc_chg_plugged_in = %d, boot_reason = %d \n", is_usb_chg_plugged_in(chip), is_dc_chg_plugged_in(chip), boot_reason);
			pr_info("[TRKL_CHG] Set default current to %d \n", m);
		} else {
			m = 1;
		}
	}
...
/* END: kidong0420.kim@lge.com 2011-10-24 */
	...
	if (batt_trkl_threshold < TRKL_BATTERY_THRESHOLD) {
		if (is_usb_chg_plugged_in(chip) && boot_reason == PWR_ON_EVENT_USB_CHG) {
			pr_info("[TRKL_CHG] In trkl_usb_booting case, time delay 2 sec\n");
			msleep(2000);
		} else {
			pr_info("[TRKL_CHG] In trkl_dc_booting case, not delay \n");
		}
...
static void monitor_boost_ic(struct work_struct *work)
{
...
#ifdef CONFIG_MACH_LGE_FX3Q_TMUS
	static int before_vbat_avg_mv;

	if (is_factory_cable())
		goto not_operate_boost_ic;

	if (boost_enable_flag == false) {
		if (before_vbat_avg_mv > BOOST_ENABLE_THRESHOLD) 
		goto not_operate_boost_ic;
	}
#endif
	...
	vbat_avg_mv = vbat_avg_mv / vbat_avg_num_samples;

	pr_info("[BOOST_IC] vbat_avg_mv: %d (vbatt: %d)\n", vbat_avg_mv, vbat_mv);
...
/* Battery alarm for the model using BMS */
#if 0 //defined(CONFIG_LGE_PM) && !defined(CONFIG_BATTERY_MAX17043)
