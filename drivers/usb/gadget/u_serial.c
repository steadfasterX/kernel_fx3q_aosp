...
static void usb_debugfs_init(struct gs_port *ui_dev, int port_num)
{
	 ...
	 * CTS issue fixed. depends on TD6007 (INTERNAL_TEST, SP_CTS)
	 * if CONFIG_DEBUG_FS = y then
	 * This change allows write only access by root.
	 * Original code :debugfs_create_file("reset", 0222, dent, ui_dev, &debug_rst_ops);
	 */
