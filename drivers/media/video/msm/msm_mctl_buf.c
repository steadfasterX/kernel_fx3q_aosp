...
static void msm_vb2_ops_buf_cleanup(struct vb2_buffer *vb)
{
	...
	pmctl = msm_cam_server_get_mctl(pcam->mctl_handle);
		/*LGE_CHANGE, qct patch for mutex lock crash, 2013-01-19, kwangsik83.kim@lge.com*/
	...
	struct ion_client *client, int domain_num)
{
	...
	for (i = 0; i < meta_frame->frame.num_planes; i++) {
		...
		if (ion_map_iommu(client, meta_frame->map[i].handle,
				domain_num, 0, SZ_4K,
				0, &paddr, &len, UNCACHED, ION_IOMMU_UNMAP_DELAYED) < 0) { /*LGE_CHANGE, fixed recording stop is not completed  0 -> ION_IOMMU_UNMAP_DELAYED, 2013-09-05, soojong.jin@lge.com */
