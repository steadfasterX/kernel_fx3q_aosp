...
/*LGE_CHANGE_S, Return to LGE Recovery, 2013-08-26, soojong.jin@lge.com */
#if 0  
...
#endif
/*LGE_CHANGE_E, Return to LGE Recovery, 2013-08-26, soojong.jin@lge.com */
			wait_count--;
//		else
//			pr_err("MSM_V4L2_CLOSE is skipped\n"); /*LGE_CHANGE, temporary code for debugging 2013-07-25, soojong.jin@lge.com */
		if (rc != -ERESTARTSYS)
			break;
		pr_err("%s: wait_event interrupted by signal, remain_count = %d",   /*LGE_CHANGE, temporary code for debugging 2013-07-25, soojong.jin@lge.com */
			__func__, wait_count);
	} while (wait_count > 0);
	...
	if (list_empty_careful(&queue->list)) {
		...
		if (rc < 0) {
			...
			pr_err("%s: current state %ld flags %d\n",__func__, current->state, current->flags);
...
#if defined(CONFIG_MACH_MSM8930_FX3) || defined(CONFIG_MACH_APQ8064_GKKT) || defined(CONFIG_MACH_APQ8064_GKSK) || defined(CONFIG_MACH_APQ8064_GKU) || defined(CONFIG_MACH_APQ8064_GKATT) || defined(CONFIG_MACH_APQ8064_GVDCM) || defined(CONFIG_MACH_LGE_L9II_COMMON)
