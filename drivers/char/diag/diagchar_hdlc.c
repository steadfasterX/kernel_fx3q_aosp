	...
	if (hdlc && hdlc->src_ptr && hdlc->dest_ptr &&
		...
		for (i = 0; i < src_length; i++) {
			...
			if (hdlc->escaping) {
				...
				/* [LGE_CHANGE_S], donghyuk.yang@lge.com, 2012-12-07, for SPRINT_SLATE_Client auto detection(skip first 7e of diag cmd) */
				#if (defined(CONFIG_MACH_LGE_FX3_SPCS) || defined(CONFIG_MACH_LGE_FX3_SPCSTRF)) // [LGE_CHANGE], Add TRACFONE_SLATE, 2013-02-15, donghyuk.yang@lge.com
				if (len == 0) {
					continue;
				}
				#endif
